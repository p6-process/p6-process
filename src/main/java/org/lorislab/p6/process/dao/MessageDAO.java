...
public class MessageDAO {
    ...
    public Uni<Long> createTokenMessage(Transaction tx, String ref) {
        return tx.preparedQuery("INSERT INTO TOKEN_MSG (ref) VALUES ($1) RETURNING (id)", Tuple.of(ref))
                .onItem().apply(pgRowSet -> pgRowSet.iterator().next().getLong("id"));
    }
    ...
    public Uni<Long> createProcessMessage(Transaction tx, String ref) {
        return tx.preparedQuery("INSERT INTO PROCESS_MSG (ref) VALUES ($1) RETURNING (id)", Tuple.of(ref))
                .onItem().apply(pgRowSet -> pgRowSet.iterator().next().getLong("id"));
    }

    public Uni<Long> createProcessMessage(Transaction tx, Message m) {
        return tx.preparedQuery("INSERT INTO PROCESS_MSG (ref) VALUES ($1) RETURNING (id)", Tuple.of(m.ref))
                .onItem().apply(pgRowSet -> pgRowSet.iterator().next().getLong("id"));
    }

    public Uni<Long> createProcessMessage(Message m) {
        return client.preparedQuery("INSERT INTO PROCESS_MSG (ref) VALUES ($1) RETURNING (id)", Tuple.of(m.ref))
                .onItem().apply(pgRowSet -> pgRowSet.iterator().next().getLong("id"));
    }
